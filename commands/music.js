...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // Récupère le player attaché au client (initialisé dans index.js)
    ...
    switch (subcommand) {
      case 'play': {
        ...
        // Forcer FFmpeg : s'assurer que dispatcherConfig existe et que skipFFmpeg est à false
        if (!track.dispatcherConfig) track.dispatcherConfig = {};
        track.dispatcherConfig.skipFFmpeg = false;

        // Création ou récupération de la queue en passant uniquement l'ID du channel dans les métadonnées
